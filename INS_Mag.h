...
#define INS_MAG_AVERAGED_MEASURES 10.0f //default number of measures used to average the readings when calibrating
...
public:
	...
	INS_Mag(MagnetometerSensor &sensor, float &meas_x, float &meas_y, float &meas_z, INS_orientation orientation); //constructor
    ...
    uint8_t cal_compute(INS_Mag_cal calibration_mode=INS_Mag_cal_9_param, uint8_t print_flag=1, SerialUSBDriver& serial_int=SDU1); //compute values, mode 0: 9 parameters, mode 1: 6 parameters, print_flag 0: no print, print_flag=1 prints the result
    uint8_t calibrate(SerialUSBDriver& serial_int, uint8_t number_of_measures=INS_MAG_CALIBRATION_MEASURES, INS_Mag_cal calibration_mode=INS_Mag_cal_9_param, uint32_t timeout=INS_MAG_TIMEOUT, uint8_t print_flag=1); //complete calibration procedure using single readings, mode 0: 9 parameters, mode 1: 6 parameters, print_flag 0: no print, print_flag=1 prints the result
    uint8_t calibrate_average(SerialUSBDriver& serial_int, uint8_t number_of_measures=INS_MAG_CALIBRATION_MEASURES, uint8_t average_length=INS_MAG_AVERAGED_MEASURES, INS_Mag_cal calibration_mode=INS_Mag_cal_9_param, uint32_t timeout=INS_MAG_TIMEOUT, uint8_t print_flag=1);   //complete calibration using averaged readings, mode 0: 9 parameters, mode 1: 6 parameters, print_flag 0: no print, print_flag=1 prints the result
#endif
private:
	MagnetometerSensor &_sens;
	float* _x_p;
	float* _y_p;
